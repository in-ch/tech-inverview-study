...
데이터베이스에서 수행되는 작업의 기본 단위이다.
트랜잭션은 데이터베이스 시스템에서 데이터의 일관성과 안전성을 보장하기 위해 사용된다. 
트랜잭션이라는 용어는 ACID 속성을 따르는 작업의 논리적인 단위를 나타낸다.
...
![스크린샷 2024-01-19 오후 11 01 30](https://github.com/in-ch/tech-inverview-study/assets/49556566/14499d14-a649-4c16-9b0a-fd7ebd95f4f0)
...
# 프로시져란 ?
데이터베이스에 대한 일련의 작업을 정리한 절차를 관계형 데이터베이스 관리 시스템에 저장한 것으로 영구저장모듈(Persistent Storage Module)이라고도 한다.
일련의 쿼리를 마치 하나의 함수처럼 실행하기 위한 쿼리의 집합이다.

### 특징
1. 리턴값이 있을 수도, 없을 수도 있다.
2. 리턴값이 여러개 가능하다.

### 장점
1. 하나의 요청으로 여러 SQL문을 실행할 수 있다.
2. 네트워크 부하를 줄일 수 있다.
3. API처럼 여러 어플맄이션과 공유가 가능하다.
4 기능 변경이 간단하다.
