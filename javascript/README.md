...
function outerFunc() {
  var x = 10;
  var innerFunc = function () { console.log(x); }; // 외부 변수 x를 사용할 수 있다.
  innerFunc();
}
...
리액트의 fiber 아키텍쳐는 reconciliation을 진행할 때 render phase와 commit phase의 두 단계로 나누어 진행한다.
...
```jsx
import { Component1Component, Component2Component, Service1Service, Service2Service } from './barrel.module';
```
...
즉시실행함수는 한 번의 실행 이후 없어지기 때문에 단 한번의 사용한 필요한 함수에 사용된다. 대표적인 예시로는 **변수를 초기화 하는 함수**가 있다. 또 **자바스크립트 모듈**을 만들때에도 즉시실행함수가 많이 활용된다.
